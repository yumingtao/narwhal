  ...
  return <main className="app-shell">
    ...
    <section className="layout">
      <aside className="sidebar">
        <button className="new-task" disabled={!workspace} onClick={() => setTaskDraft(true)}><Icon name="plus"/>New task</button>
        <SideBar workbench={workbench} conversation={conversation} workspace={workspace} agent={agent} selectedTask={selectedTask} choose={() => void mutate(api.chooseWorkspace)} selectWorkspace={(id) => void mutate(() => api.selectWorkspace(id))} createSession={() => void agentCall(api.createSession)} selectSession={(id) => void agentCall(() => api.selectSession(id))} selectTask={(id) => void mutate(() => api.selectTask(id))}/>
      ...
      <aside className={workbench.panelOpen ? 'context-panel open' : 'context-panel'}><div className="panel-head"><div><p>Work context</p><h2>{selectedTask?.title ?? 'No task selected'}</h2></div><button title="Close panel" onClick={() => void mutate(() => api.setPanelOpen(false))}><Icon name="close"/></button></div>{!workspace ? <p className="panel-empty">Choose a workspace to keep its plan, changed files and deliverables together.</p> : <><TaskSection task={selectedTask} onSelect={(taskId) => void mutate(() => api.selectTask(taskId))} onNew={() => setTaskDraft(true)} onUpdate={(input) => void mutate(() => api.updateTask(input))} onTodo={(taskId, todoId, done) => void mutate(() => api.toggleTodo({ taskId, todoId, done }))} onAddTodo={(taskId, text) => void mutate(() => api.addTodo({ taskId, text }))}/><ChangesSection changes={workbench.git.changes}/><DeliverablesSection entries={workbench.deliverables} onNew={() => setDeliverableDraft(true)} onReveal={(path) => void api.revealDeliverable(path)} onRemove={(path) => void mutate(() => api.unpinDeliverable(path))}/></>}</aside>
    </section>
    {taskDraft && <TaskDialog close={() => setTaskDraft(false)} save={(title, goal) => mutate(() => api.createTask({ title, goal })).then(() => setTaskDraft(false))}/>} 
...
function TaskSection({ task, onSelect, onNew, onUpdate, onTodo, onAddTodo }: { task?: Task; onSelect: (id: string) => void; onNew: () => void; onUpdate: (input: { taskId: string; title?: string; goal?: string; status?: 'active' | 'done' }) => void; onTodo: (taskId: string, todoId: string, done: boolean) => void; onAddTodo: (taskId: string, text: string) => void }) { const [editing, setEditing] = useState(false); const [step, setStep] = useState(''); const complete = task?.todos.filter((item) => item.done).length ?? 0; const total = task?.todos.length ?? 0; useEffect(() => setEditing(false), [task?.id]); if (!task) return <section className="panel-section"><div className="section-title"><span>Plan</span><button onClick={onNew}><Icon name="plus"/></button></div><button className="quiet-add" onClick={onNew}>Create the first task</button></section>; return <section className="panel-section"><div className="section-title"><span>Plan</span><span><button title="Edit task" onClick={() => setEditing(!editing)}><Icon name="settings"/></button><button onClick={onNew}><Icon name="plus"/></button></span></div>{editing ? <div className="task-edit"><input defaultValue={task.title} aria-label="Task title" onBlur={(event) => event.target.value.trim() && onUpdate({ taskId: task.id, title: event.target.value })}/><textarea defaultValue={task.goal} aria-label="Task goal" onBlur={(event) => event.target.value.trim() && onUpdate({ taskId: task.id, goal: event.target.value })}/><label><input type="checkbox" checked={task.status === 'done'} onChange={(event) => onUpdate({ taskId: task.id, status: event.target.checked ? 'done' : 'active' })}/>Mark task complete</label></div> : <button className="task-choice" onClick={() => onSelect(task.id)}>{task.goal}</button>}<div className="progress"><span style={{ width: `${total ? complete / total * 100 : 0}%` }}/></div><small>{complete}/{total} steps complete</small><div className="todos">{task.todos.length ? task.todos.map((todo) => <label key={todo.id}><input type="checkbox" checked={todo.done} onChange={(event) => onTodo(task.id, todo.id, event.target.checked)}/><span>{todo.text}</span></label>) : <p className="muted">No steps yet.</p>}<form className="add-step" onSubmit={(event) => { event.preventDefault(); if (step.trim()) { onAddTodo(task.id, step); setStep('') } }}><input value={step} onChange={(event) => setStep(event.target.value)} placeholder="Add a step"/><button aria-label="Add step"><Icon name="plus"/></button></form></div></section> }
...
function TaskDialog({ close, save }: { close: () => void; save: (title: string, goal: string) => Promise<void> }) { const [title, setTitle] = useState(''); const [goal, setGoal] = useState(''); return <Dialog title="New task" close={close}><label>Task title<input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Refine the macOS release"/></label><label>Goal<textarea value={goal} onChange={(event) => setGoal(event.target.value)} placeholder="What does success look like?"/></label><button className="primary" disabled={!title.trim() || !goal.trim()} onClick={() => void save(title, goal)}>Create task</button></Dialog> }
