...
import type { AgentConfiguration, AgentConversation, AgentSnapshot, CreateProviderResult, Deliverable, DesktopSettings, GitChange, Task, TaskStatus, TodoItem, WorkbenchSnapshot, Workspace } from '../shared/desktop-contract.js'
...
interface StoredWorkspace { id: string; path: string; name: string; lastOpenedAt: string; tasks: Task[]; selectedTaskId?: string; selectedSessionId?: string; deliverables: Deliverable[]; panelOpen: boolean }
...
async function snapshot(): Promise<WorkbenchSnapshot> {
  ...
  return { workspaces: store.workspaces.map(workspaceSummary), selectedWorkspaceId: store.selectedWorkspaceId, tasks: workspace?.tasks ?? [], selectedTaskId: workspace?.selectedTaskId, deliverables: workspace?.deliverables ?? [], panelOpen: workspace?.panelOpen ?? store.panelOpen ?? false, git: await inspectGit(workspace), conversation: hostBridge.snapshot() }
}
...
async function chooseWorkspace(): Promise<WorkbenchSnapshot> {
  ...
  if (!workspace) { workspace = { id: randomUUID(), path, name: basename(path), lastOpenedAt: now, tasks: [], deliverables: [], panelOpen: true }; store.workspaces.unshift(workspace) }
...
function registerIpc(): void {
  ...
  ipcMain.handle('narwhal:create-task', async (event, raw) => { sender(event); const value = asRecord(raw); const workspace = requireWorkspace(); const now = new Date().toISOString(); const task: Task = { id: randomUUID(), title: asString(value.title, 'title', 120), goal: asString(value.goal, 'goal', 1200), status: 'active', todos: [], createdAt: now, updatedAt: now }; workspace.tasks.unshift(task); workspace.selectedTaskId = task.id; await persist(); return snapshot() })
  ipcMain.handle('narwhal:update-task', async (event, raw) => { sender(event); const value = asRecord(raw); const workspace = requireWorkspace(); const taskId = asString(value.taskId, 'taskId', 100); const task = workspace.tasks.find((item) => item.id === taskId); if (!task) throw new Error('Task not found'); const status = value.status; if (status !== undefined && status !== 'active' && status !== 'done') throw new Error('Invalid task status'); workspace.tasks = workspace.tasks.map((item) => item.id === taskId ? { ...item, ...(value.title !== undefined && { title: asString(value.title, 'title', 120) }), ...(value.goal !== undefined && { goal: asString(value.goal, 'goal', 1200) }), ...(status !== undefined && { status: status as TaskStatus }), updatedAt: new Date().toISOString() } : item); await persist(); return snapshot() })
  ipcMain.handle('narwhal:add-todo', async (event, raw) => { sender(event); const value = asRecord(raw); const workspace = requireWorkspace(); const taskId = asString(value.taskId, 'taskId', 100); const task = workspace.tasks.find((item) => item.id === taskId); if (!task) throw new Error('Task not found'); const todo: TodoItem = { id: randomUUID(), text: asString(value.text, 'todo text', 240), done: false }; workspace.tasks = workspace.tasks.map((item) => item.id === taskId ? { ...item, todos: [...item.todos, todo], updatedAt: new Date().toISOString() } : item); await persist(); return snapshot() })
  ipcMain.handle('narwhal:toggle-todo', async (event, raw) => { sender(event); const value = asRecord(raw); const done = value.done; const workspace = requireWorkspace(); const taskId = asString(value.taskId, 'taskId', 100); const task = workspace.tasks.find((item) => item.id === taskId); if (!task || typeof done !== 'boolean') throw new Error('Todo not found'); const todoId = asString(value.todoId, 'todoId', 100); if (!task.todos.some((item) => item.id === todoId)) throw new Error('Todo not found'); workspace.tasks = workspace.tasks.map((item) => item.id === taskId ? { ...item, todos: item.todos.map((item) => item.id === todoId ? { ...item, done } : item), updatedAt: new Date().toISOString() } : item); await persist(); return snapshot() })
  ipcMain.handle('narwhal:select-task', async (event, raw) => { sender(event); const value = asRecord(raw); const workspace = requireWorkspace(); const id = value.taskId === undefined ? undefined : asString(value.taskId, 'taskId', 100); if (id && !workspace.tasks.some((task) => task.id === id)) throw new Error('Task not found'); workspace.selectedTaskId = id; await persist(); return snapshot() })
