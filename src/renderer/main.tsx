...
function statusText(state: AgentSnapshot['state']) { return state === 'ready' ? t('settings.runtime.agentReady') : state === 'starting' ? t('settings.runtime.agentStarting') : t('settings.runtime.agentNeedsRestart') }
...
function ThemeSettings({ theme, setTheme, lang, setLang }: { theme: ThemeMode; setTheme: (t: ThemeMode) => void; lang: Lang; setLang: (l: Lang) => void }) {
  ...
  return <section className="settings-page theme-settings">
    ...
    <div className="settings-group">
      <label className="settings-label">{t('settings.language')}
...
function ProviderRow({ provider, update, models, openModels, onDelete }: { provider: AgentConfiguration['providers'][number]; update: (operation: () => Promise<AgentConfiguration>) => void; models: readonly AgentConfiguration['models'][number]['models'][number][]; openModels: (providerId: string) => void; onDelete: () => void }) {
  ...
  const capabilities = [
    provider.active ? t('settings.providers.statusAvailable') : t('settings.providers.statusInactive'),
    provider.apiKeyWritable ? t('settings.providers.statusKeyManaged') : t('settings.providers.statusNoKeyEditor'),
    provider.baseUrl !== undefined ? t('settings.providers.statusCustomEndpoint') : null,
  ].filter(Boolean)
  const hasConfig = true // Always allow configuration
  const modelCount = models.length
  const modelLabel = !modelCount ? t('settings.providers.modelCountNone') : modelCount === 1 ? t('settings.providers.modelCountOne') : t('settings.providers.modelCountMany', { count: modelCount })
  const addModelId = () => setModelIds((prev) => [...prev, ''])
  const removeModelId = (index: number) => setModelIds((prev) => prev.filter((_, i) => i !== index))
  const updateModelId = (index: number, value: string) => setModelIds((prev) => prev.map((m, i) => (i === index ? value : m)))
  const validModelIds = modelIds.map((m) => m.trim()).filter(Boolean)
    ...
    {expanded && <div className="provider-row-config">
        ...
        <div className="model-ids-list">
          {modelIds.map((modelId, index) => (
            <div key={index} className="model-id-row">
              ...
              {modelIds.length > 1 && <button type="button" className="remove-model" onClick={() => removeModelId(index)} aria-label={t('settings.providers.removeModelAria', { index: index + 1 })}>×</button>}
            </div>
          ))}
        </div>
        <button type="button" className="add-model-btn" onClick={addModelId}>{t('settings.providers.addModel')}</button>
      </label>
      {provider.apiKeyWritable && <label>{t('settings.providers.apiKeyLabel')}
        <div className="field-action">
          <input type="password" autoComplete="off" value={key} onChange={(event) => setKey(event.target.value)} placeholder={provider.apiKeyConfigured ? t('settings.providers.replaceKey') : t('settings.providers.pasteKey')}/>
      ...
      <div className="config-actions">
        <button type="button" className="primary" onClick={handleSaveConfig} disabled={saving || !validModelIds.length}>
          {saving ? t('settings.providers.saving') : t('settings.providers.saveChanges')}
        </button>
      </div>
    </div>}
  </article>
}
function PermissionSettings({ configuration, update }: { configuration: AgentConfiguration; update: (operation: () => Promise<AgentConfiguration>) => void }) { return <section className="settings-page"><p>{t('settings.permissions.newConvPermSub')}</p>{configuration.permissionOptions.length ? <label>{t('settings.permissions.newConvPerm')}<select value={configuration.defaultPermission ?? ''} onChange={(event) => { const next = event.target.value; if (next.toLowerCase().includes('full') && !window.confirm(t('settings.permissions.fullConfirm'))) return; update(() => api.setDefaultPermission(next)) }}>{configuration.permissionOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select></label> : <p className="settings-empty">{t('settings.permissions.unavailable')}</p>}</section> }
function RuntimeSettings({ settings, agent, restart }: { settings: DesktopSettings; agent: AgentSnapshot; restart: () => void }) { return <section className="settings-page"><div className="settings-status"><span className={`status-dot ${agent.state}`}/><div><strong>{statusText(agent.state)}</strong><small>{t('settings.runtime.localAgent')}</small></div></div><dl className="settings-list"><div><dt>{t('settings.runtime.appVersion')}</dt><dd>{settings.appVersion}</dd></div><div><dt>{t('settings.runtime.runtime')}</dt><dd>{settings.runtimeVersion}</dd></div><div><dt>{t('settings.runtime.workbenchData')}</dt><dd title={settings.dataDirectory}>{settings.dataDirectory}</dd></div></dl><button className="primary" type="button" onClick={restart} disabled={agent.state === 'starting'}>{agent.state === 'starting' ? t('settings.runtime.startingAgent') : t('settings.runtime.restartAgent')}</button></section> }
